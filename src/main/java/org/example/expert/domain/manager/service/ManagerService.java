...
public class ManagerService {
    ...
    public ManagerSaveResponse createManager(AuthUser authUser, Long todoId, ManagerSaveRequest managerSaveRequest) {
        ...
        if (user.getId() == null || todo.getUser() == null || !user.getId().equals(todo.getUser().getId())) {
            throw new InvalidRequestException("담당자를 등록하려고 하는 유저가 일정을 만든 유저가 유효하지 않습니다.");
        }

        User managerUser = userRepository.findById(managerSaveRequest.managerUserId()).orElseThrow(
